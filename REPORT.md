...
- `tests/`：9 个测试文件，**44 个离线用例**（全走 mock，不调真实 codex）
- `skill/SKILL.md`：技能正文（触发条件 / 硬性边界 / spec 格式 / 子代理身份写法）
- `README.md`：含任务 1 探针的环境结论

## 测试与验证
- **离线**：`python -m unittest discover -s tests -v` → **44 tests OK**（主环境，Claude 复跑，非只信派工方）。
- **真实冒烟**（`D:\.codex-tmp\workflows\smoke-review-runner-20260613-074834`）：3 个只读子代理
...
- **意外收获**：冒烟子代理审出 runner.py 三个问题，前两个已修（见下），第三个记为已知限制。
...
## 哪些没测
- 并发 4 的极限；真实冒烟只验了并发 2、单条两阶段流水线。
- `taskkill` 进程树清理只有逻辑、无自动化测试。
- Windows 保留名只测了校验拦截，没测「真用 CON 跑会崩」（已被校验挡在前面）。
