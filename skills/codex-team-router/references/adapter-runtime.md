...
3. Send executor work with `send_executor_dispatch_with_adapter()`. The dispatch delegates the complete outcome and requires autonomous understand -> internal plan -> RED -> implement -> focused tests -> self-review before one terminal role return. It must include `callbackMode: self-thread-marker` and `TEAM_ROUTER_CALLBACK taskId=<taskId>`. When `returnThreadId` is available, also include `callbackDelivery: direct-send`.
...
Do not use continuation prompts or micro-dispatches to steer a normally running Executor. If host write capability fails, the role must return `blocked` with the exact error and `zero-write + clean worktree` evidence. Broker-recovery is Manager-owned and may begin only after that failure is recorded; its work is not evidence of Executor success, worktree-writer success, or hard permission.
...
Version 1 compatibility registry persistence uses `update_registry_roles()` and `create_team_task()` to record the `manager`, `executor`, and `verifier` thread ids under the project registry before task dispatch. Task creation writes `tasks/<taskId>.json` immediately and moves the ledger to `roles_ready`. V2 state uses the project-level shared pool `projects[projectId].managerPools["__project__"]`, delayed task-ledger creation only for delegated work, and no child Manager binding.
...
`read-only/design-only 不是沙箱`. These are prompt boundaries, not enforceable filesystem or API sandboxes. If the original user goal asks for workspace writes, continue only when the manager dispatch explicitly grants an authorized `local-package` executor scope and the route includes Verifier; Reviewer is added only for STRICT/PACKAGE risk or an explicit Reviewer/QA gate. Unsupported or high-risk writes still stop before dispatch: commit/push/PR/merge/deploy, global config, project-local `AGENTS.md`, destructive operations, real APIs, accounts, or production data require separate explicit authorization outside ordinary Team Router dispatch.

Team Router dispatch messages may use `read-only`, `design-only`, or `local-package`. `local-package` authorizes executor-delegated workspace writes only within the stated package; it does not authorize manager direct edits, commits, pushes, global config changes, production/API actions, or destructive operations. Manager Mode requests such as `优化 skill`, `改规则`, `修`, `继续`, or `复利` authorize only a dispatch proposal. They do not authorize `create_thread`, role messages, registry/ledger writes, implementation, or actual `DISPATCH_ONLY`; that requires an explicit current-turn create/dispatch request. A specific manager-side file edit still requires the separate role and file-edit authorization described above.
