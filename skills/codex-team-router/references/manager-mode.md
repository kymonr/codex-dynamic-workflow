...
Closeout reporting policy: every task closeout must report implemented changes, verification actually run and results, blockers/exceptions, remaining risks, and current state and next step. At closeout, make a compounding decision: record a reusable lesson when there is manager overreach, role conflict, permission/sandbox issue, test instability, temp-file/workspace pollution, or the user explicitly adds a reusable process preference. Do not record ordinary successful implementation/testing with no new reusable risk.
