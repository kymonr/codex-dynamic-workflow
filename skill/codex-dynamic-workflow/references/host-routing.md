...
backend handoff and one authority/budget ledger. Persistent graphs and transactional launch reservations are available only in the
explicit Runtime mode; automatic resume, fee caps and nested orchestration are not.
Read [Runtime protocol](runtime.md) before using its native bridge or exec adapter.
