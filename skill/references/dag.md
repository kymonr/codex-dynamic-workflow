...
Serial native writing is the default. Sol is the default native/delegated writer, while a user-explicit supported native model selection takes precedence and may select Luna as the one scoped writer. Root never writes the same candidate concurrently with that writer. Non-selected Luna branches and explicit Grok second-review tasks are read-only. Identity mismatch, unresolved critical identity, ambiguous effects, or invalid review state returns to root and blocks only dependent nodes.
