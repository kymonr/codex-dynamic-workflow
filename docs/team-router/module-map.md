...
This records the conservative phase 1 split plus the phase 2b1 runtime extraction and phase 2b2 direct-return contract extraction now implemented. Public imports continue through `src/team_router.py`; direct imports from the extracted modules are internal implementation detail unless a later explicit package widens that contract.
...
- facade and contract snapshot: `src/team_router.py` remains the public import surface and keeps `protocol_contract_snapshot()` because the snapshot aggregates marker schema from protocol, gate policy from policy, and still-local role/state/orchestration/side-effect/closeout contracts. It re-exports runtime and direct-return helpers for compatibility.
...
- adapter runtime: `src/team_router_runtime.py` owns adapter callable dispatch helpers, send-anchor normalization, create-result thread-id extraction, and `read_thread` message normalization. `src/team_router.py` still owns host readiness, live orchestration gating, watcher timing, and public compatibility imports.
...
| `team_router.py` | public facade, state/capture/watch/state-save/host readiness/watcher/status, `protocol_contract_snapshot()` | `team_router_protocol`, `team_router_policy`, `team_router_runtime`, `team_router_direct_return`, standard library | re-exports moved names for compatibility |
...
| `team_router_host_runtime.py` | host readiness, live orchestration context, watcher/heartbeat timing | readiness, heartbeat, read discipline tests | no live dispatch claim without explicit readiness evidence |
