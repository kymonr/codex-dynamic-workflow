...
- facade and contract snapshot: `src/team_router.py` remains the public import surface and keeps `protocol_contract_snapshot()` because the snapshot aggregates marker schema from protocol, gate policy from policy, and still-local role/state/orchestration/side-effect/closeout contracts. It re-exports runtime, direct-return, and host runtime helpers for compatibility.
...
| `team_router.py` | public facade, state/capture/watch/state-save/watcher/status, `protocol_contract_snapshot()` | `team_router_protocol`, `team_router_policy`, `team_router_runtime`, `team_router_direct_return`, `team_router_host_runtime`, standard library | re-exports moved names for compatibility |
