...
- facade and contract snapshot: `src/team_router.py` remains the public import surface and keeps `protocol_contract_snapshot()` because the snapshot aggregates marker schema from protocol, gate policy from policy, and still-local role/state/orchestration/side-effect contracts. It re-exports runtime, direct-return, host runtime, watcher runtime, and status/closeout helpers for compatibility.
