...
This records the conservative phase 1 split plus the phase 2b1 runtime extraction now implemented. Public imports continue through `src/team_router.py`; direct imports from the extracted modules are internal implementation detail unless a later explicit package widens that contract.
...
- facade and contract snapshot: `src/team_router.py` remains the public import surface and keeps `protocol_contract_snapshot()` because the snapshot aggregates marker schema from protocol, gate policy from policy, and still-local role/state/orchestration/side-effect/closeout contracts. It re-exports runtime helpers for compatibility.
...
| `team_router.py` | public facade, state/direct-return/host readiness/watcher/status, `protocol_contract_snapshot()` | `team_router_protocol`, `team_router_policy`, `team_router_runtime`, standard library | re-exports moved names for compatibility |
