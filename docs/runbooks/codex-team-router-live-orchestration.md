...
For authorized delegation, derive the SHA-256 `targetFingerprint` from canonical JSON `{"hostId": hostId, "target": target}` before pool state. Reuse only the matching manager-owned pool record and carry `parallelAllowed` into reservation. New roles bootstrap with Luna Medium; the formal dispatch always sends resolved `model` + `thinking`. A creation intent can bind one uniquely verified bootstrap identity; zero/multiple candidates terminally record `creation_outcome_unknown` and never auto-create a duplicate.
...
- Adapter-created orchestration requires the host to pass the current parent thread id as `parent_thread_id`. V1 renames it to `调度者-Team Router <task label>`; V2 authorized delegation uses `管理者-Team Router <task label>` after preflight. If the host lacks that id or callable `set_thread_title`, return `tool_error` / blocked.
...
V2 Role reuse policy: reuse only from the same manager-owned `parentThreadId + hostId + targetFingerprint + role` pool. Rework prefers the original role, and a busy role gets a new same-role thread only when `parallelAllowed` is true. Create a replacement only when the old role is unavailable/archived/broken/invalid or the isolation boundary changed. Version 1 compatibility: for the same `taskId` or task family, reuse existing executor, existing reviewer when the conditional reviewer gate applies, and existing verifier threads by default. Rework goes back to the original executor thread, rework review goes back to the original reviewer thread, and rework verification goes back to the original verifier thread; a changed isolation requirement is a replacement reason.
