...
Start a normal Manager task with `你作为管理者，完成 <目标>`. This standard entry allows Manager direct or planning, but it does not select a concrete role model. For delegated default routing use `你作为管理者，按 Luna Medium、Terra Medium、Sol High 成本感知路由完成 <目标>`; it authorizes Luna Medium (`gpt-5.6-luna + medium`), Terra Medium (`gpt-5.6-terra + medium`), and Sol High (`gpt-5.6-sol + high`). Sol Ultra is forbidden.

The parent Manager owns the V2 resolved plan; V2 never creates or messages a child Manager. Resolve authorization -> effective gate -> route closure -> model authorization before readiness, title, heartbeat, ledger, registry, or thread calls. Manager direct returns without ledger/state. A delegated route without model authorization returns `model_authorization_required` at the same boundary. Use visible Codex role threads only; never fall back to native `spawn_agent` or collaboration subagents.

For authorized delegation, derive the SHA-256 `targetFingerprint` from canonical JSON `{"hostId": hostId, "target": target}` before pool state. Active-binding pool identity is `projectId + hostId + targetFingerprint + role`; it excludes `parentThreadId`. Only the exact active `taskId + requestId` binding may resume. Without a trusted execution-domain issuer/field/version/lifetime, idle Role reuse is disabled and release/terminal cleanup retires the pool record; do not invent `executionDomainKey`. Carry `parallelAllowed` into reservation. New roles bootstrap with Luna Medium; the formal dispatch always sends resolved `model` + `thinking`. Creation-intent identity additionally includes the current `parentThreadId` and can bind one uniquely verified bootstrap identity; zero/multiple candidates terminally record `creation_outcome_unknown` and never auto-create a duplicate.

Multiple ready execution units with no dependencies and disjoint write sets must fan out to separate Executors in independent worktrees. Overlapping writes, ordering dependencies, or permission conflicts disable parallel execution.

FAST/NORMAL Manager direct uses no role. FAST/NORMAL delegated work ends through Manager acceptance; explicit Reviewer/QA closes with Verifier; STRICT/PACKAGE is Executor -> Reviewer -> Verifier. Automatic model upgrade and automatic rework each allow one task-wide attempt. A V2 closeout prints `routingReceipt` for requested model/thinking and tool acceptance only: requested model is not actual model or billing/cost evidence.

sideEffectTaxonomy: classify manager actions as `READ_ONLY`, `DISPATCH_ONLY`, `LOCAL_CLOSEOUT`, `WORKSPACE_WRITE`, `HEAVY_OR_RISKY`, or `EXTERNAL_RELEASE` before acting. In active Manager Mode, `可以`, `修`, `继续`, `开始修`, `先修`, `修这个`, and `do it` authorize only a dispatch proposal, not actual `DISPATCH_ONLY` or implementation. `READ_ONLY` supports manager judgment and low-frequency/event-driven `read_thread` only. `WORKSPACE_WRITE` requires explicit `local-package` executor delegation and required gates unless the user explicitly switches roles with “切回执行者”, “你亲自改代码”, or “按这个 plan 落地”, plus explicit current-turn user authorization for manager file edits. `LOCAL_CLOSEOUT` requires verifier pass plus explicit user commit request, may stage only accepted files, and excludes unrelated untracked plus push/PR/merge/deploy. `EXTERNAL_RELEASE` requires separate publish/release authorization. `HEAVY_OR_RISKY` requires explicit separate authorization. A named reviewer for Team Router self changes still requires a visible reviewer role conversation; subagent fallback is not allowed.
...
callbackDeliveryModel: use `direct-send + self-thread-marker fallback`; the complete result schemas/templates are owned only by `skills/codex-team-router/references/direct-return.md`. A text-only delegation wrapper's `<source_thread_id>` is recorded only as untrusted `delegatedSourceThreadId`. Matching Host-structured `content[].codexDelegation` is normalized to trusted `agentMessage + sourceThreadId`; conflicting or incomplete structured metadata remains untrusted. Manager-inbox consumption requires that normalized Host provenance, a canonical Host item id, and source metadata matching the pending Role thread. An ordinary `userMessage` is not a consumable Role result even when its body contains a complete strict tuple. Protocol-block `sourceThreadId` must still match pending `returnThreadId`, and `sourceRoleThreadId` must match the pending Role thread. Self-thread fallback additionally requires Host `agentMessage` plus a canonical Host item id. Each terminal Role result is at most 1200 UTF-8 bytes with no line-count limit. Unmatched or untrusted blocks are rejected or quarantined and cannot expand scope. Bare `create_thread` plus `read_thread` is not formal dispatch or completion.

## Preconditions

- The parent Host can call the core tools `list_projects`, `list_threads`, `create_thread`, `send_message_to_thread`, and `read_thread`; it supplies explicit `parent_thread_id`, trusted Host sender/source identity, and trusted execution-domain identity.
...
- Adapter-created orchestration requires the host to pass the current parent thread id as `parent_thread_id`. Missing scheduler limits the path to `interactive_contract_ready`; a callable scheduler permits `unattended_contract_ready`. `set_thread_title` is optional best-effort normalization and its absence/failure is warning-only. Callable/snapshot evidence never proves live operation; only explicit `codex-desktop-live` call counts may produce a `*_live_verified` tier.
...
| 规划者 | Manager | parent in V2; yes in V1 | Version 2 parent owns the plan; Version 1 replies with `TEAM_ROUTER_PLAN`. |
...
Version 1 compatibility: 规划者 / 执行者 / 验证者 are default core role threads. 审查者 is a conditional reviewer role thread and should be created or reused only when its gate applies. The parent-side concepts must not create extra threads.

Title rules are versioned. V1 uses parent `调度者-Team Router <task label>` and role `角色-任务名`; `TeamRouter <role> - <projectId>` is discovery/migration-only. V2 uses parent `管理者-Team Router <task label>` and pooled role `角色-Team Router <projectId>`; one temporary parallel role may append `#2` without changing pool identity `projectId + hostId + targetFingerprint + role`.

Executor dispatch is complete-outcome delegation: understand -> internal plan -> RED -> implement -> focused tests -> self-review, then one terminal role return (`done`, `needs_feedback`, or `blocked`). The Manager does not send continuation prompts or micro-dispatches during normal work. `TEAM_ROUTER_CALLBACK.status` remains `done | blocked`. A host write-capability failure must return `blocked` with the exact error and `zero-write + clean worktree`; only the Manager may explicitly perform broker-recovery, and that recovery is not Executor success, parallel writer success, isolated-worktree write, or hard permission.
...
Use `parent_entry_guard()` at the parent boundary before choosing the adapter-created path. If the Host lacks core callable thread tools, explicit `parent_thread_id`, trusted sender/source identity, or trusted execution-domain identity—or the adapter only exposes model-side tool descriptors—the adapter runner is not available. Missing scheduler means interactive-only; missing title support is warning-only. Continue through the manual/pre-created path when the hard gate is not met.

Use `orchestrate_team_task_with_adapter()` when the parent Host satisfies the hard gate. The helper probes core tools, best-effort normalizes titles, resolves the current project target with `list_projects`, discovers current active bindings with `list_threads`, starts a missing task, advances the next send/read step, and returns `action`, `status`, `ledger`, `userOutput`, `capabilities`, `readiness`, `codexProjectId`, and `projectTarget`. It is not the entrypoint for pre-created role threads when the Host lacks core callable tools or trusted identity.
...
Manager waiting policy: `read_thread` polling is allowed only as bounded, low-frequency, event-driven waiting. Direct-send return is preferred, and watcher/heartbeat `read_thread` is fallback. Run one observation-only first check at ledger `firstCheckAt`; after that, ordinary proactive fallback reads must respect `nextAllowedReadAt` and remain at least 300 seconds apart. User-triggered status/stop/immediate, a known completion window, timeout, or blocker handling may justify an earlier read. Forbid continuous polling and mid-run instruction injection. Active role states are normal processing: do not restart, replace, or send a shorter delta prompt, do not repeat unchanged status, and emit only one timeout notice before an intervention decision.

Fast Lane policy: classify Team Router work as FAST, NORMAL, STRICT, or PACKAGE. V2 FAST/NORMAL can be Manager direct; delegated FAST/NORMAL ends at Manager acceptance unless explicit Reviewer/QA closes to Verifier. V2 STRICT/PACKAGE is executor -> reviewer -> verifier. Version 1 compatibility: FAST covers docs/BOM/single phrase rework, routes executor -> verifier, and NORMAL covers small focused code/test work, routes executor -> verifier. Completion is direct-return first; bounded read_thread fallback is allowed only after the 300 second minimum class window, user-triggered status request, known expected completion window, or timeout/blocker handling.
V2 Role resume policy: resume only the same active `taskId + requestId` binding under `projectId + hostId + targetFingerprint + role`. Without trusted execution-domain evidence, do not reuse idle Roles. Rework within the active task prefers the original role, and a busy role gets a new same-role thread only when `parallelAllowed` is true. Archived/broken/invalid Roles are unavailable.
...
3. Version 1 compatibility: let the helper reuse registry role bindings and call `create_thread` through the adapter only for missing manager/executor/verifier core role threads; create or reuse reviewer only when the conditional reviewer gate applies. V2 uses manager-owned pools and never creates a Manager role.
4. Version 1 compatibility: let the helper persist registry role bindings and create the task ledger. V2 persists the resolved parent plan and manager-pool state only after delegated preflight passes.
...
The fixture `tests/fixtures/team_router/live_manager_inbox_direct_return.json` is sanitized transport-shape evidence only. Tests must prove that a text-only wrapper `<source_thread_id>` remains untrusted `delegatedSourceThreadId`, while matching Host-structured `content[].codexDelegation` is promoted to normalized `agentMessage + sourceThreadId` and numeric timestamps remain available for recovery filtering. An ordinary manager-inbox `userMessage` must not advance the ledger.
...
The expected stable fields are `status: ready`, `orchestrationStatus: unattended_contract_ready`, and `adapterInjection.threadToolCallsExecuted: 0`. This is evidence-only contract readiness: the synthetic callable adapter raises if any represented tool is executed, so it cannot claim a `*_live_verified` tier.
...
The expected stable fields are `status: blocked`, `orchestrationStatus: manual/pre-created`, and missing core callable/identity evidence in `doctorHostReadiness.missing`. Scheduler absence by itself is interactive-only rather than blocked; title absence is warning-only.
